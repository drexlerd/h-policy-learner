...
IW::IW(const Options &opts)
    ...
    assert(m_child_search_engines.size() == 0);
    m_name = "IW";
    m_current_width = m_iterate ? 0 : m_width;
}

bool IW::is_novel(const State &state) {
...
bool IW::is_novel(const OperatorProxy& op, const State &state) {
...
void IW::print_statistics() const {
    statistics.print_detailed_statistics();
    m_search_space->print_statistics();
}

SearchStatus IW::step() {
    /* Search exhausted */
    ...
    if (id == m_initial_state_id) {
        if (is_goal(state)) {
            m_solution = IWSolution{{}, state.get_id(), m_current_width};
    ...
    for (auto op_id : applicable_ops) {
        ...
        if (is_goal(succ_state)) {
            ...
            m_solution = IWSolution{plan, succ_state.get_id(), m_current_width};
            if (!m_parent_search_engine) {
                solution_found = true;
                cout << "Average effective width: " << m_current_width << endl;
                cout << "Maximum effective width: " << m_current_width << endl;
            }
            return SearchStatus::SOLVED;
        }
    }
...
void IW::set_state_registry(std::shared_ptr<StateRegistry> state_registry) {
...
void IW::set_propositional_task(std::shared_ptr<extra_tasks::PropositionalTask> m_propositional_task) {
    HierarchicalSearchEngine::set_propositional_task(m_propositional_task);
}

bool IW::set_initial_state(const State& state) {
...
void IW::dump_search_space() const {
    m_search_space->dump(task_proxy);
}

IWSolutions IW::get_partial_solutions() const {
    return {m_solution,};
}
