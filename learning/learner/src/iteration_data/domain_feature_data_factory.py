...
class DomainFeatureDataFactory:
    ...
    def make_domain_feature_data(self, config, domain_data: DomainData, dlplan_states: List[dlplan.State]):
        ...
        # To use features from parent node for free have add them with cost 2 and add cost + 2 to each generated feature
        for boolean_feature in boolean_features:
            ...
            domain_feature_data.boolean_features.add_feature(Feature(boolean_feature, boolean_feature.compute_complexity() + 2 + 1))
            domain_data.all_domain_feature_data.boolean_features.add_feature(Feature(boolean_feature, boolean_feature.compute_complexity() + 2 + 1))
        for numerical_feature in numerical_features:
            domain_feature_data.numerical_features.add_feature(Feature(numerical_feature, numerical_feature.compute_complexity() + 2))
            domain_data.all_domain_feature_data.numerical_features.add_feature(Feature(numerical_feature, numerical_feature.compute_complexity() + 2))
