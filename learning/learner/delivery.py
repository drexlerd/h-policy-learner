...
def experiments():
    ...
    exps["sketch_debug"] = update_dict(
        ...
        instance_filenames=training_instances(),
...
def training_instances():
    instance_filenames = []
    for instance_name in [f"instance_{h}_{w}_{n}_{seed}.pddl" for h in range(1,4) for w in range(1,4) for n in range(1,3) for seed in range(0,1) ]:
        instance_filenames.append(BENCHMARK_DIR / "delivery" / "instances" / instance_name)
